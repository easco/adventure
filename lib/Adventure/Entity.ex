defmodule Adventure.Entity do
  def start_link(id, components, opts \\ []) when is_atom(id) do
    ...
    Enum.each(components, fn {component_id, component_value} ->
      update_component(new_entity, component_id, just_put(component_value))
    end)
  ...
  def handle_call({:update_component, component_id, update_function}, _from, {
        entity_id,
        components
      }) do
...

    {
      ...
      {entity_id, Map.put(components, component_id, new_component)}
    }
  end
  ...
  defp get_reply(component_id, component_value, state),
    do: {:reply, {component_id, component_value}, state}
end
